...
## 2026-05-16: 데이터 삭제 기준 산정 — 단지별 MAX(billing_ym) + YoY 고려

**상황**: "12개월 차트이므로 12개월치 보존" → 202504 이전 삭제 계획 수립. 그러나 일부 단지는 MAX(billing_ym)=202602여서 12개월 back=202503 필요. 또한 YoY 비교를 위해 202502도 필요.

**올바른 계산**:
- 차트 범위: MAX billing_ym 기준 -12개월
- MAX가 단지별로 다를 수 있음 → 최솟값 MAX 기준으로 계산
- YoY 1개월 추가 버퍼 필요
- K-APT 딜레이 2개월 → 실제 보존 공식: `현재월 - 15개월` 이전 삭제

**교훈**: 삭제 기준은 "서비스가 실제로 사용하는 가장 오래된 데이터"를 단지별로 계산해야 함.

---

## 2026-05-16: 병렬 수집 시 이미 완전한 월은 제외

**상황**: 12개월 병렬 수집 실행 → 202601·202602(100% 완전)도 재수집. API 낭비.

**올바른 접근**: 수집 전 `GROUP BY billing_ym`으로 현황 파악 → 100% 완전한 월은 TARGET_YMS에서 제외.

**교훈**: 병렬 수집 실행 전 D1 현황 조회 필수. 수집 대상을 최소화해야 API 낭비 없음.
