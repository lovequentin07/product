...
async function getD1MgmtFeeResult(
  ...
  try {
    const ranks = await db.batch([
      ...
      /* 4 umd_rank     */ umd ? db.prepare(`SELECT COUNT(*)+1 as v FROM apt_mgmt_fee WHERE ${BASE} AND sgg_nm=? AND umd_nm=? AND total_per_hh < ?`).bind(billing_ym, sgg, umd, total) : noRow,
      /* 5 umd_total    */ umd ? db.prepare(`SELECT COUNT(*) as v FROM apt_mgmt_fee WHERE ${BASE} AND sgg_nm=? AND umd_nm=?`).bind(billing_ym, sgg, umd) : noRow,
