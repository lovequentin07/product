    ...
    <div className="mx-auto max-w-2xl p-4">
      ...
      <main className="mt-8 space-y-10">
        ...
        <section>
          <Link
            href="/market"
            className="block w-full rounded-xl bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 px-6 py-4 text-center text-white font-semibold transition-colors"
          >
            📊 전체 시세 정보 보기
          </Link>
        </section>
        ...
        {/* 보조 서비스 (축소) */}
        <section className="border-t border-gray-100 dark:border-gray-800 pt-6">
          <h2 className="text-xs font-semibold text-gray-400 dark:text-gray-600 uppercase tracking-wide mb-2">
            기타 서비스
          </h2>
          <div className="space-y-2">
            {secondaryServices.map((svc) => (
              <a
                key={svc.href}
                href={svc.href}
                className="flex items-center gap-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-3 py-2 hover:border-gray-300 dark:hover:border-gray-600 transition-colors"
              >
                <span className="text-lg shrink-0">{svc.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium text-gray-700 dark:text-gray-300">{svc.title}</div>
                </div>
